...
class TestAntiDetection:
    ...
    async def test_fingerprint_generation(self):
        ...
        assert fingerprint.timezone in [
            "Europe/Madrid",
            "Europe/Barcelona",
            "Europe/Valencia",
        ]
    ...
    def test_human_delay_generation(self):
        ...
        assert (
            scraper_config.MIN_DELAY <= delay <= scraper_config.MAX_DELAY + 5
        )  # +5 para variaciones
...
class TestErrorHandler:
    ...
    def test_error_recording(self):
        ...
        error_handler.record_error(
            test_error, {"test": "context"}, ErrorSeverity.MEDIUM
        )
...
class TestUtilities:
    ...
    def test_text_cleaner(self):
        ...
        for text, expected in zip(price_texts, expected_prices):
            ...
            assert (
                extracted == expected
            ), f"Failed for '{text}': got {extracted}, expected {expected}"
    ...
    def test_data_validator(self):
        ...
        for phone in invalid_phones:
            assert not DataValidator.is_valid_phone(
                phone
...
class TestScraperIntegration:
    ...
    async def test_mock_message_processing(self):
        ...
        with patch("src.scraper.scraper_integration.DatabaseManager") as mock_db:
            ...
            with patch.object(
                integration, "_save_message_to_db", return_value=AsyncMock()
            ):
                with patch.object(integration, "_create_buyer_object"):
                    with patch.object(integration, "_create_product_object"):
                        with patch.object(
                            integration.scraper, "send_message", return_value=True
                        ):
                            with patch.object(integration, "_save_response_to_db"):
                                result = await integration._process_single_message(
                                    test_message, mock_conversation
                                )
...
class TestRealWorldScenarios:
    ...
    def test_suspicious_message_detection(self):
        ...
        for msg in suspicious_messages:
            assert DataValidator.is_suspicious_message(
                msg
            ), f"Should detect as suspicious: {msg}"

        for msg in legitimate_messages:
            assert not DataValidator.is_suspicious_message(
                msg
            ), f"Should NOT detect as suspicious: {msg}"
    ...
    def test_conversation_flow_simulation(self):
        ...
        product_mentioned = any(
            product.titulo.lower() in resp.lower() for resp in responses
        )
