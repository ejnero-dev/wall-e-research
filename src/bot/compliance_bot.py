...
class ComplianceWallapopBot:
    ...
    def __init__(self, config_path: str = "config/config.yaml"):
        ...
        if not self.config.get('development', {}).get('gdpr_compliance', {}).get('enabled', False):
        ...
        try:
            ...
            while True:
                ...
                if response == 's':
                    ...
                    self._log_audit_entry("human_approval", action.target_user, 
                                        {"action": action.action_type.value, "approved": True}, 
                                        automated=False, human_confirmed=True)
                    return True
                    
                elif response == 'n':
                    print("❌ Acción RECHAZADA por humano")
                    self._log_audit_entry("human_rejection", action.target_user,
                    ...
                    print("❓ Respuesta inválida. Use: s (sí), n (no), p (pausar), q (salir)")
    ...
    def _check_rate_limits(self) -> bool:
        ...
        if time_since_last < 120:  # 2 minutos
            logger.warning(f"[COMPLIANCE] Minimum delay not met ({time_since_last}s < 120s)")
            return False
        
        return True
    
    def _log_audit_entry(self, action: str, user_id: str, details: Dict, 
                        automated: bool = True, human_confirmed: bool = False):
    ...
    async def _monitor_messages(self):
        ...
        while self.is_running:
            try:
                        ...
                        if conversation.get('unread_count', 0) > 0:
                            ...
                            for message in messages:
                                if not message.get('is_from_me', False) and not message.get('is_processed', False):
                                    ...
                                    await asyncio.sleep(self.config.get('wallapop', {}).get('behavior', {}).get('min_delay_between_messages', 120))
                ...
                await asyncio.sleep(max(60, self.config.get('wallapop', {}).get('behavior', {}).get('message_check_interval', 60)))
    ...
    async def _process_message(self, message: Dict):
        ...
        try:
            ...
            self._log_audit_entry("message_processed", buyer_id, message, automated=True)
            ...
            if analysis.get('fraud_risk', 0) > 30 or analysis.get('intention') == 'fraude':
                analysis['requires_human_review'] = True
            else:
                # En modo compliance, revisar menos críticos pero con threshold bajo
                analysis['requires_human_review'] = analysis.get('requires_human', False) or (analysis.get('fraud_risk', 0) > 10)
            
            # 6. Generar respuesta sugerida usando Conversation Engine
            suggested_response = None
            if not analysis.get('fraud_risk', 0) > 80:  # No generar respuestas para fraude obvio
            ...
            if analysis['requires_human_review']:
                action = PendingAction(
                    ...
                    details={
                        ...
                        'analysis': analysis,
                        'suggested_response': suggested_response or "Respuesta requiere revisión manual"
                    },
                ...
                logger.info(f"[COMPLIANCE] Action {action.id} created and pending human confirmation")
                await self._notify_human_required(action)
            
        except Exception as e:
            logger.error(f"[COMPLIANCE] Error processing message: {e}")
            self._log_audit_entry("error", buyer_id or "unknown", {"error": str(e)}, automated=True)
    ...
    async def _queue_response(self, buyer_id: str, response: str, human_confirmed: bool = False):
        ...
        if not human_confirmed and self.config['wallapop']['behavior']['require_human_confirmation']:
            logger.warning(f"[COMPLIANCE] Response to {buyer_id} requires human confirmation")
            return
        
        # Verificar consentimiento GDPR
        if not await self._verify_gdpr_consent(buyer_id):
            logger.warning(f"[COMPLIANCE] Cannot send response to {buyer_id} - no GDPR consent")
            return
        
        delay = self._calculate_response_delay()
        
        # Añadir disclosure de automatización
        disclosure = self.config.get('security', {}).get('automation_disclosure', {}).get('message', '')
        ...
        self._log_audit_entry("response_queued", buyer_id, {
            "response_length": len(response),
            "delay": delay,
            "human_confirmed": human_confirmed
        }, automated=not human_confirmed, human_confirmed=human_confirmed)
    ...
    def _check_rate_limits(self) -> bool:
        ...
        max_per_hour = self.config['wallapop']['behavior'].get('max_messages_per_hour', 5)
        if self.hourly_message_count >= max_per_hour:
            logger.warning(f"[COMPLIANCE] Hourly message limit ({max_per_hour}) reached")
            return False
    ...
    async def _verify_gdpr_consent(self, user_id: str) -> bool:
        ...
        if not self.config.get('development', {}).get('gdpr_compliance', {}).get('consent_required', True):
            return True
        ...
        retention_days = self.config.get('development', {}).get('gdpr_compliance', {}).get('data_retention_days', 30)
    ...
    def _log_audit_entry(self, action: str, user_id: str, details: Dict, automated: bool = True, human_confirmed: bool = False):
        """Registra entrada en audit trail"""
        entry = AuditLogEntry(
            timestamp=datetime.now(),
            action=action,
            ...
            automated=automated,
            human_confirmed=human_confirmed,
            gdpr_compliant=self._verify_gdpr_consent(user_id) if user_id != "unknown" else True
        )
    ...
    async def _notify_human_required(self, action: PendingAction):
        ...
        logger.info(f"[HUMAN REQUIRED] Action {action.id} - {action.action_type.value} for {action.target_user}")
    ...
    async def confirm_action(self, action_id: str, approved: bool, human_user: str) -> bool:
        ...
        if approved:
            ...
            if action.action_type == ActionType.SEND_MESSAGE:
                ...
                await self._queue_response(action.target_user, response, human_confirmed=True)
            
            self._log_audit_entry("action_approved", action.target_user, {
                "action_id": action_id,
                "approved_by": human_user
            }, automated=False, human_confirmed=True)
        else:
            logger.info(f"[COMPLIANCE] Action {action_id} rejected by {human_user}")
            self._log_audit_entry("action_rejected", action.target_user, {
                "action_id": action_id,
