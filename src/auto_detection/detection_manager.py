...
class DetectionManager:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Auto-detection system started with {self.config['scan_interval_minutes']}min intervals"
            )
            return True
    ...
    async def _on_product_changed(
        self, old_product: DetectedProduct, new_product: DetectedProduct
    ):
        ...
        if old_product.status != new_product.status:
            changes.append(
                f"status: {old_product.status.value} → {new_product.status.value}"
            )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.info(
                            f"Successfully added product to dashboard: {result.get('id', 'unknown')}"
                        )
                        self.stats["products_auto_added"] += 1
                    else:
                        error_text = await response.text()
                        logger.error(
                            f"Failed to add product to dashboard: {response.status} - {error_text}"
                        )
                        self.stats["api_errors"] += 1
    ...
    async def _update_product_in_dashboard(
        self, product: DetectedProduct, changes: List[str]
    ):
        ...
        try:
            ...
            dashboard_product_id = await self._find_product_id_by_url(
                product.wallapop_url
            )

            if not dashboard_product_id:
                logger.warning(
                    f"Product not found in dashboard, adding as new: {product.title}"
                )
                await self._add_product_to_dashboard(product)
                return
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(
                    ...
                    if response.status == 200:
                        logger.info(
                            f"Successfully updated product in dashboard: {dashboard_product_id}"
                        )
                    else:
                        error_text = await response.text()
                        logger.error(
                            f"Failed to update product in dashboard: {response.status} - {error_text}"
                        )
                        self.stats["api_errors"] += 1
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(
                    ...
                    if response.status == 200:
                        logger.info(
                            f"Successfully marked product as removed: {dashboard_product_id}"
                        )
                    else:
                        error_text = await response.text()
                        logger.error(
                            f"Failed to mark product as removed: {response.status} - {error_text}"
                        )
                        self.stats["api_errors"] += 1
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(
                            f"Failed to get products from dashboard: {response.status}"
                        )
    ...
    async def _send_notification(self, notification_type: str, data: Dict[str, Any]):
        ...
        try:
            ...
            if notification_type == "new_product":
                ...
                await notification_manager.notify_scanner_error(
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            "success_rate": (
                (
                    self.stats["products_auto_added"]
                    / max(self.stats["products_detected"], 1)
                )
                * 100
                if self.stats["products_detected"] > 0
                else 0
            ),
            "api_success_rate": (
                (
                    (self.stats["api_calls_made"] - self.stats["api_errors"])
                    / max(self.stats["api_calls_made"], 1)
                )
                * 100
