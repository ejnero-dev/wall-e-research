...
class NotificationManager:
    ...
    def enable_channel(self, channel: NotificationChannel, enabled: bool = True):
        ...
        if channel_name in self.config:
            ...
            logger.info(f"{'Enabled' if enabled else 'Disabled'} {channel_name} notifications")
    ...
    async def _send_through_channel(self, channel: NotificationChannel, notification: Dict[str, Any]):
    ...
    async def _send_slack(self, notification: Dict[str, Any]):
        ...
        try:
            ...
            if notification.get("data") and notification["type"] == "new_product":
                ...
                attachment = {
                    "color": "good" if notification["priority"] == "normal" else "warning",
                    "fields": [
                        {"title": "Price", "value": f"€{data.get('price', 'N/A')}", "short": True},
                        {"title": "URL", "value": data.get('url', 'N/A'), "short": False}
                    ]
                }
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(webhook_url, json=slack_message) as response:
                    if response.status == 200:
                        logger.debug(f"Sent Slack notification: {notification['title']}")
                    else:
    ...
    async def _send_console(self, notification: Dict[str, Any]):
        ...
        try:
            ...
            if notification.get("data") and notification["type"] in ["new_product", "product_changed"]:
                data = notification["data"]
                if "price" in data:
...
"""
    ...
    async def notify_product_changed(self, product_data: Dict[str, Any], changes: List[str]):
    ...
    async def notify_system_status(self, status: str, details: Dict[str, Any]):
        ...
        priority = NotificationPriority.HIGH if status in ["error", "critical"] else NotificationPriority.NORMAL
