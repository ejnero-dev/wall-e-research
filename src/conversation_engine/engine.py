...
class ConversationEngine:
    def __init__(self, templates_path: str = None):
        ...
        if templates_path is None:
            templates_path = (
                Path(__file__).parent.parent / "templates" / "responses.json"
            )
    ...
    def _init_intention_keywords(self) -> Dict:
        ...
        return {
            ...
            IntentionType.DISPONIBILIDAD: [
                "disponible",
                "vendido",
                "reservado",
                "queda",
            ],
            IntentionType.ESTADO_PRODUCTO: [
                "estado",
                "funciona",
                "roto",
                "nuevo",
    ...
    def _calculate_priority(
        self, intention: IntentionType, buyer: Buyer, message: str
    ) -> BuyerPriority:
        ...
        if buyer.valoraciones == 0 and buyer.num_compras == 0:
            return BuyerPriority.BAJA
        if intention == IntentionType.NEGOCIACION and any(
            x in message for x in ["10€", "20€", "mitad"]
        ):
            return BuyerPriority.BAJA
    ...
    def _detect_conversation_state(
        self, intention: IntentionType, buyer_id: str
    ) -> ConversationState:
    ...
    def generate_response(
        self, analysis: Dict, message: str, buyer: Buyer, product: Product
    ) -> Optional[str]:
        ...
        if response_category:
            response = self._select_response_template(
                response_category, analysis["state"]
            )
    ...
    def _get_security_response(self, message: str) -> str:
        ...
        if any(word in message_lower for word in ["whatsapp", "telegram", "teléfono"]):
            return random.choice(
                self.templates.get("respuestas_seguridad", {}).get(
                    "no_whatsapp", ["Prefiero mantener la comunicación por Wallapop"]
                )
            )
        ...
        if "adelantado" in message_lower or "por adelantado" in message_lower:
            return random.choice(
                self.templates.get("respuestas_seguridad", {}).get(
                    "pago_anticipado", ["El pago se hace en la entrega"]
                )
            )
        ...
        if re.search(r"https?://", message_lower):
            return random.choice(
                self.templates.get("respuestas_seguridad", {}).get(
                    "no_enlaces", ["No puedo acceder a enlaces externos"]
                )
            )
        ...
        return random.choice(
            self.templates.get("respuestas_seguridad", {}).get(
                "sospecha_fraude", ["Lo siento, no me parece seguro"]
            )
        )
    ...
    def _select_response_template(self, category: str, state: ConversationState) -> str:
        ...
        if category == "saludos":
            subcategory = (
                "inicial" if state == ConversationState.INICIAL else "respuesta"
            )
        ...
        if category in self.templates:
            if isinstance(self.templates[category], dict) and subcategory:
                ...
                return (
                    random.choice(self.templates[category])
                    if self.templates[category]
                    else ""
                )
    ...
    def _personalize_response(
        self, template: str, product: Product, buyer: Buyer
    ) -> str:
    ...
    def should_respond(self, buyer: Buyer, last_message_time: datetime) -> bool:
        ...
        min_delay = (
            self.templates.get("variables_sistema", {})
    ...
    def handle_conversation_flow(
        self, buyer_id: str, messages: List[Dict], product: Product
    ) -> Dict:
        ...
        if conv["state"] == ConversationState.ABANDONADO:
            ...
            if conv["messages"] < 10:
                ...
                if recovery_msg:
                    return {"action": "recuperar", "message": recovery_msg}
    ...
    def _get_recovery_message(self, last_activity: datetime) -> Optional[str]:
        ...
        if hours_passed < 24:
            ...
            return random.choice(
                self.templates.get("recuperacion_venta", {}).get("24h", [])
            )
        else:
            return random.choice(
                self.templates.get("recuperacion_venta", {}).get("48h", [])
            )
    ...
    def get_conversation_summary(self, buyer_id: str) -> Dict:
        ...
        return {
            ...
            "requires_attention": conv["state"]
            in [ConversationState.COMPROMETIDO, ConversationState.COORDINANDO],
        }
