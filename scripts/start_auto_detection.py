...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/auto_detection.log'),
...
class AutoDetectionService:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self.detection_manager.set_notification_enabled(self.config["notification_enabled"])
            ...
            while retries < self.config["max_retries"]:
                try:
                    ...
                    if success:
                        ...
                        retries += 1
                        logger.warning(f"Failed to start detection manager (attempt {retries}/{self.config['max_retries']})")
                    ...
                    retries += 1
                    logger.error(f"Error starting detection manager (attempt {retries}/{self.config['max_retries']}): {e}")
    ...
    async def _health_check(self):
        ...
        try:
            if not self.detection_manager or not self.detection_manager.is_running:
                ...
                logger.info(f"Health check passed - Status: {status['detection_manager']['is_running']}")
    ...
    def get_status(self):
        ...
        if not self.detection_manager:
            return {
                "service": "stopped",
                "detection_manager": "not_initialized"
            }
