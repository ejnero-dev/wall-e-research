...
class ScraperOrchestrator:
    ...
    async def _verify_configuration(self):
        ...
        if scraper_config.MAX_CONCURRENT_CONVERSATIONS > 10:
            self.logger.warning(
                "⚠️  High concurrent conversation limit may trigger detection"
            )
    ...
    def _determine_auth_method(self) -> AuthMethod:
        ...
        if credentials_file.exists():
            self.logger.info(
                "🔑 Found stored credentials, using credential authentication"
            )
    ...
    async def _monitor_system_health(self):
        ...
        try:
            ...
            if recent_errors > 50:  # Más de 50 errores en 24h
                self.logger.warning(
                    f"⚠️  High error rate detected: {recent_errors} errors in 24h"
                )
    ...
    async def _attempt_recovery(self):
        ...
        try:
            if self.integration and self.integration.scraper:
                ...
                session_info = (
                    await self.integration.scraper.session_manager.refresh_session(
                ...
                if session_info:
                    ...
                    self.logger.warning(
                        "⚠️  Session refresh failed, may need re-authentication"
                    )

        except Exception as e:
            self.logger.error(f"❌ Recovery attempt failed: {e}")
            error_handler.record_error(
                e, {"context": "recovery_attempt"}, ErrorSeverity.HIGH
            )
    ...
    async def _generate_stats_report(self):
        ...
        try:
            ...
            if "integration" in stats:
                ...
                self.logger.info(
                    f"  - Active conversations: {integration_stats.get('active_conversations', 0)}"
                )
                self.logger.info(
                    f"  - Messages processed: {integration_stats.get('processed_messages', 0)}"
                )

            if "errors" in stats:
                error_stats = stats["errors"]
                self.logger.info(
                    f"  - Total errors: {error_stats.get('total_errors', 0)}"
                )
                self.logger.info(
                    f"  - Recent errors (24h): {error_stats.get('recent_errors_24h', 0)}"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Wallapop Scraper - Sistema de automatización"
    )
    parser.add_argument("--config", help="Archivo de configuración personalizado")
    parser.add_argument(
        "--auth-method",
        choices=["cookies", "credentials", "auto"],
        default="auto",
        help="Método de autenticación",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
