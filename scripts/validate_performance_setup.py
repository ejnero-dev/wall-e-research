...
def check_redis_availability() -> Tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            return (
                True,
                f"Redis available (version {info.get('redis_version', 'unknown')})",
            )
        except Exception as e:
...
def print_status(
    check_name: str, success: bool, message: str, details: List[str] = None
):
...
def main():
    ...
    print_status(
        ...
        (
            ...
            if packages_ok
            else f"Missing {len(missing_packages)} packages"
        ),
    ...
    print_status(
        ...
        (
            ...
            if modules_ok
            else f"Missing {len(missing_modules)} modules"
        ),
    ...
    if "error" not in system_info:
        meets_requirements = (
            ...
            and system_info["total_ram_gb"] >= 8
            and system_info["disk_free_gb"] >= 10
        )
        ...
        print_status(
            ...
            (
                ...
                else "System below minimum requirements"
            ),
            [
        ...
        print_status(
            "System Resources", False, f"Error checking system: {system_info['error']}"
        )
