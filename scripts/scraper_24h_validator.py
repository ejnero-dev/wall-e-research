...
class ScraperValidator:
    ...
    def __init__(self, test_duration_hours: float = 24.0):
        ...
        self.detailed_log = (
            self.results_dir
            / f"detailed_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
    ...
    async def run_24h_validation(self) -> ValidationMetrics:
        ...
        self.logger.info(
            f"Starting 24h validation - Duration: {self.test_duration_hours} hours"
        )
    ...
    async def _run_validation_tests(self):
        ...
        self.logger.info(
            f"Running validation tests for {self.test_duration_hours} hours"
        )
        ...
        while datetime.now() < end_time and self.monitoring_active:
            try:
                ...
                if elapsed.total_seconds() % 3600 < 60:  # Cada hora aprox
                    ...
                    self.logger.info(
                        f"Validation progress: {hours_elapsed:.1f}h completed"
                    )
    ...
    async def _test_session_validity(self):
        ...
        try:
            if self.integration and self.integration.scraper:
                session_info = (
                    self.integration.scraper.session_manager.get_session_info()
                )

                if (
                    not session_info
                    or not self.integration.scraper.session_manager._is_session_valid()
                ):
        ...
        try:
            ...
            if len(self.action_timestamps) >= 2:
                ...
                if len(recent_actions) > 1:
                    ...
                    if time_span > 0:
                        actions_per_minute = (len(recent_actions) - 1) / (
                            time_span / 60
                        )
                        ...
                        if actions_per_minute > 2.5:  # Margen de error
                            self.logger.warning(
                                f"High action rate detected: {actions_per_minute:.2f}/min"
                            )
            ...
            if self.response_times:
                ...
                self.metrics.average_response_time = sum(recent_times) / len(
                    recent_times
                )
    ...
    async def _generate_interim_report(self):
        ...
        interim_file = (
            self.results_dir / f"interim_metrics_{datetime.now().strftime('%H%M')}.json"
        )
    ...
    def _calculate_final_metrics(self):
        ...
        if len(self.action_timestamps) > 1 and elapsed.total_seconds() > 0:
            ...
            self.metrics.actions_per_minute = (
                len(self.action_timestamps) / total_minutes
            )
    ...
    def _evaluate_success_criteria(self):
        ...
        self.metrics.continuous_operation = self.metrics.total_runtime_hours >= (
            self.test_duration_hours * 0.95
        )  # 95% del tiempo
        ...
        total_attempts = (
            self.metrics.total_messages_processed + self.metrics.total_errors
        )
        if total_attempts > 0:
        ...
        self.metrics.realistic_speed = (
            self.metrics.actions_per_minute <= 2.1
        )  # Margen de error

        # Criterio 5: Recuperación automática ante errores
        self.metrics.error_recovery = (
            self.metrics.automatic_recoveries > 0 or self.metrics.total_errors < 10
        )
    ...
    async def _generate_final_report(self):
        ...
        with open(report_file, "w") as f:
            ...
            f.write("-" * 30 + "\n")
            f.write(
                f"✓ Continuous Operation (24h): {'PASS' if self.metrics.continuous_operation else 'FAIL'}\n"
            )
            f.write(
                f"✓ Zero Detections: {'PASS' if self.metrics.zero_detections else 'FAIL'}\n"
            )
            f.write(
                f"✓ Messages Processed Correctly: {'PASS' if self.metrics.messages_processed_correctly else 'FAIL'}\n"
            )
            f.write(
                f"✓ Realistic Speed: {'PASS' if self.metrics.realistic_speed else 'FAIL'}\n"
            )
            f.write(
                f"✓ Error Recovery: {'PASS' if self.metrics.error_recovery else 'FAIL'}\n\n"
            )
            ...
            f.write(
                f"OVERALL RESULT: {'✓ PASS - All criteria met' if all_passed else '✗ FAIL - Some criteria not met'}\n\n"
            )

            f.write("DETAILED METRICS:\n")
            f.write("-" * 20 + "\n")
            f.write(
                f"Total Messages Processed: {self.metrics.total_messages_processed}\n"
            )
            f.write(
                f"Total Conversations Handled: {self.metrics.total_conversations_handled}\n"
            )
            f.write(
            ...
            f.write(
                f"Average Delay Between Actions: {self.metrics.average_delay_between_actions:.2f}s\n"
            )
            ...
            f.write(
                f"Circuit Breaker Activations: {self.metrics.circuit_breaker_activations}\n"
            )
            f.write(
                f"Successful Health Checks: {self.metrics.successful_health_checks}\n"
            )
    ...
    def _log_current_status(self):
        ...
        self.logger.info(
            f"Conversations handled: {self.metrics.total_conversations_handled}"
        )
        self.logger.info(f"Actions/min: {self.metrics.actions_per_minute:.2f}")
        self.logger.info(f"Errors: {self.metrics.total_errors}")
        self.logger.info(
            f"Health checks: {self.metrics.successful_health_checks}✓/{self.metrics.failed_health_checks}✗"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validador 24h del scraper de Wallapop"
    )
    parser.add_argument(
        "--duration",
        type=float,
