...
class HappyPathDemo:
    ...
    async def simulate_conversation(self, messages: List[str]):
        ...
        for message in messages:
            ...
            # Analizar mensaje
            analysis = self.engine.analyze_message(message, self.buyer, self.product)
            ...
            response = self.engine.generate_response(
                analysis, message, self.buyer, self.product
            )
    ...
    def display_conversation_summary(self):
        ...
        if summary["exists"]:
            ...
            print(
                f"⚠️  Requiere atención: {'Sí' if summary['requires_attention'] else 'No'}"
            )
