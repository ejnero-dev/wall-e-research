...
class ComplianceValidator:
    ...
    def _validate_config_compliance(self):
        ...
        if not config_path.exists():
            self.results.append(ValidationResult(
                "Configuration File", ComplianceLevel.CRITICAL, False,
        ...
        try:
            ...
            if behavior.get('human_confirmation_required') != True:
                self.results.append(ValidationResult(
                    "Human Confirmation", ComplianceLevel.CRITICAL, False,
                    "Human confirmation not enabled",
                    "Set wallapop.behavior.human_confirmation_required = true"
                ))
            else:
                self.results.append(ValidationResult(
                    "Human Confirmation", ComplianceLevel.PASS, True,
            ...
            if behavior.get('transparency_disclosure') != True:
                self.results.append(ValidationResult(
                    "Transparency Disclosure", ComplianceLevel.CRITICAL, False,
                    "Transparency disclosure not enabled",
                    "Set wallapop.behavior.transparency_disclosure = true"
                ))
            else:
                self.results.append(ValidationResult(
                    "Transparency Disclosure", ComplianceLevel.PASS, True,
                    "Transparency properly configured"
                ))
                
        except Exception as e:
            self.results.append(ValidationResult(
                "Configuration Parsing", ComplianceLevel.CRITICAL, False,
    ...
    def _validate_rate_limits(self):
        ...
        try:
            ...
            if max_messages_hour > 5:
                self.results.append(ValidationResult(
                    "Hourly Rate Limit", ComplianceLevel.CRITICAL, False,
                    f"Rate limit too high: {max_messages_hour}/hour (max: 5)",
                    "Reduce max_messages_per_hour to 5 or less"
                ))
            else:
                self.results.append(ValidationResult(
                    "Hourly Rate Limit", ComplianceLevel.PASS, True,
    ...
    def _validate_anti_detection_disabled(self):
        ...
        try:
            ...
            if anti_detection.get('enabled') != False:
                self.results.append(ValidationResult(
                    "Anti-Detection Disabled", ComplianceLevel.CRITICAL, False,
                    "Anti-detection features must be disabled",
                    "Set anti_detection.enabled = false"
                ))
            else:
                self.results.append(ValidationResult(
                    "Anti-Detection Disabled", ComplianceLevel.PASS, True,
            ...
            if evasion.get('webdriver_detection_bypass') != False:
                self.results.append(ValidationResult(
                    "WebDriver Bypass Disabled", ComplianceLevel.CRITICAL, False,
            ...
            if browser.get('user_agent_rotation') != False:
                self.results.append(ValidationResult(
                    "User Agent Rotation Disabled", ComplianceLevel.HIGH, False,
            ...
            self.results.append(ValidationResult(
                "Anti-Detection Validation", ComplianceLevel.CRITICAL, False,
    ...
    def _validate_human_oversight(self):
        ...
        try:
            ...
            if oversight.get('enabled') != True:
                self.results.append(ValidationResult(
                    "Human Oversight Enabled", ComplianceLevel.CRITICAL, False,
                    "Human oversight system must be enabled",
                    "Set human_oversight.enabled = true"
                ))
            else:
                self.results.append(ValidationResult(
                    "Human Oversight Enabled", ComplianceLevel.PASS, True,
            ...
            if missing_triggers:
                self.results.append(ValidationResult(
                    "Escalation Triggers", ComplianceLevel.HIGH, False,
            ...
            self.results.append(ValidationResult(
                "Human Oversight Validation", ComplianceLevel.CRITICAL, False,
    ...
    def _validate_gdpr_compliance(self):
        ...
        try:
            ...
            if gdpr.get('enabled') != True:
                self.results.append(ValidationResult(
                    "GDPR Features Enabled", ComplianceLevel.CRITICAL, False,
                    "GDPR compliance features must be enabled",
                    "Set security.gdpr_compliance.enabled = true"
                ))
            
            required_features = [
                'consent_required', 'data_minimization', 'right_to_be_forgotten'
            ]
            
            for feature in required_features:
                if gdpr.get(feature) != True:
                    self.results.append(ValidationResult(
                        f"GDPR {feature.title()}", ComplianceLevel.CRITICAL, False,
            ...
            if consent.get('enabled') != True:
                self.results.append(ValidationResult(
                    "Consent Management", ComplianceLevel.CRITICAL, False,
            ...
            self.results.append(ValidationResult(
                "GDPR Validation", ComplianceLevel.CRITICAL, False,
    ...
    def _validate_legal_documentation(self):
        ...
        for doc_name, description in required_docs.items():
            ...
            if not doc_path.exists():
                self.results.append(ValidationResult(
                    f"Legal Documentation - {doc_name}", ComplianceLevel.CRITICAL, False,
                ...
                if 'compliance' not in content.lower() and 'gdpr' not in content.lower():
                    self.results.append(ValidationResult(
                        f"Legal Content - {doc_name}", ComplianceLevel.HIGH, False,
                        f"{doc_name} lacks compliance warnings",
                        f"Add proper compliance notices to {doc_name}"
                    ))
                else:
                    self.results.append(ValidationResult(
                        f"Legal Documentation - {doc_name}", ComplianceLevel.PASS, True,
    ...
    def _validate_code_restrictions(self):
        ...
        if anti_detection_file.exists():
            ...
            if 'COMPLIANCE' not in content:
                self.results.append(ValidationResult(
                    "Anti-Detection Compliance", ComplianceLevel.HIGH, False,
            ...
            if 'webdriver_detection_bypass: false' not in content:
                if 'webdriver' in content.lower():
                    self.results.append(ValidationResult(
                        "WebDriver Bypass Check", ComplianceLevel.CRITICAL, False,
        ...
        if scraper_config.exists():
            ...
            if 'COMPLIANCE' not in content:
                self.results.append(ValidationResult(
                    "Scraper Config Compliance", ComplianceLevel.MEDIUM, False,
    ...
    def _generate_report(self) -> Tuple[bool, List[ValidationResult]]:
        ...
        for level in [ComplianceLevel.CRITICAL, ComplianceLevel.HIGH, 
                     ComplianceLevel.MEDIUM, ComplianceLevel.LOW, ComplianceLevel.PASS]:
